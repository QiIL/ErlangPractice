...
-record(event, {name="", desc="", pid, timeout={{1970, 1, 1}, {0, 0, 0}}}).
    ...
    receive
        ...
        {done, Name} ->
            case orddict:find(Name, S#state.events) of
                {ok, E} ->
                    send_to_clients({done, E#event.name, E#event.desc}, S#state.clients),
...
init() ->
    %% 从静态文件中加载时间的逻辑可以放这里
    %% 需要给init函数传递一个参数，用来指定从那个文件中寻找时间。然后即可进行加载
