...
sync_queue(Name, Args) -> gen_server:call(Name, {sync, Args}, infinity).

async_queue(Name, Args) -> gen_server:cast(Name, {async, Args}).

stop(Name) -> gen_server:call(Name, stop).

init({Limit, MFA, Sup}) ->
    %% 我们要在此处得到工作者进程监督者的pid，
    %% 但是，这需要调用监督者，而它正等待着我的回应。
